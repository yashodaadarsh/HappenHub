...
@Component
public class InternshalaScrapeJob {
        ...
        try {
            ...
            for (WebElement job : jobs) {
                ...
                try{
                    ...
                    List<WebElement> locElements = job.findElements(By.xpath(".//p[contains(@class,'locations')]//a"));
                    StringBuilder location = new StringBuilder();
                    for (int i = 0; i < locElements.size(); i++) {
                        if (i > 0) location.append(", ");
                        location.append(locElements.get(i).getText());
                    }
                    if (location.toString().toLowerCase().contains("remote")) {
                        location = new StringBuilder("Remote");
                    }
